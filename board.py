...
class Board:
    def __init__(self, rows, columns):
        ...
        self._grid = [[Cell() for column_cells in range(self._columns)] for row_cells in range(self._rows)] #creats cells according to the size we are given
    ...
    def update_board(self):
        ...
        for row in range(len(self._grid)):
            for column in range(len(self._grid[row])):
                check_neighbour = self.check_neighbour(row, column) #gets the locations of all the neighbours for a specific (row, column)

                living_neighbours_count = []

                for neighbour_cell in check_neighbour:
                    if neighbour_cell.is_alive():
                        living_neighbours_count.append(neighbour_cell) #counts how many neighbours are alive
    ...
    def draw_board(self):
        """
        method that draws the actual board.
        """
        self._count += 1
        print("generation: " +  str(self._count) )
